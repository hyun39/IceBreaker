...
# 표준 디렉토리 구조 (spec_new/std/05_infra.md 참조)
mkdir -p \
  api/app/{routers,services,schemas,core,clients} \
  api/tests/bdd/{features/analysis,steps/analysis} \
  api/migrations \
  airflow/dags \
  frontend/src \
  docs/adr \
...
로컬에서 **Compose 한 벌**로도 되고, **로컬 Kubernetes(kind / minikube / k3d)** 로도 같은 구성을 올릴 수 있다. 팀 표준·플랫폼 엔지니어링 정책에 맞게 하나를 고른다. 운영·GitOps 배포는 `spec_new/std/05_infra.md` · `specs/edit/enterprise` GitOps 문서를 따른다.
...
cd api && docker build -t sp500-api:local . && cd ..
...
- [ ] 비즈니스 스펙 작성됨 (`specs/` 내)
- [ ] .feature 파일 작성됨 (`tests/bdd/features/` 내)
...
**소요 시간**: 30분  
**참조**: `spec_new/gov/01_requirements.md`, `spec_new/bdd/templates/business_spec.md`

```bash
# 템플릿 복사
cp spec_new/bdd/templates/business_spec.md \
...
**소요 시간**: 15분  
**참조**: `spec_new/bdd/01_writing_guide.md`, `spec_new/bdd/templates/feature.feature`

```bash
cp spec_new/bdd/templates/feature.feature \
   api/tests/bdd/features/analysis/trend_analysis.feature
```
...
**예상 출력**:
```
FAILED tests/bdd/features/analysis/trend_analysis.feature
       :: 거래일 트렌드 분석 정상 조회
...
**참조**: `spec_new/bdd/02_fastapi_impl.md`

```python
# api/tests/bdd/conftest.py
import pytest
...
에이전트(ReAct·구조화 출력·실패 시나리오)까지 다루려면 **`spec_new/bdd/04_agent_impl.md`** 를 참조한다.
...
```python
# api/tests/bdd/steps/analysis/trend_analysis_steps.py
...
**소요 시간**: 2~8시간  
**참조**: `spec_new/std/01_backend.md`, `spec_new/std/03_database.md`

```python
# api/app/routers/analysis.py  (std/01_backend.md 패턴)
...
```python
# api/app/core/exceptions.py  (std/01_backend.md 에러 처리 패턴)
...
**목표 출력**:
```
PASSED tests/bdd/features/analysis/trend_analysis.feature
       :: 거래일 트렌드 분석 정상 조회
...
[x] .feature 파일 작성됨 (tests/bdd/features/analysis/trend_analysis.feature)
...
# 1. 스펙 작성
cp spec_new/bdd/templates/business_spec.md specs/next_feature_spec.md

# 2. .feature 작성
cp spec_new/bdd/templates/feature.feature \
   tests/bdd/features/{domain}/next_feature.feature
...
# 특정 feature만
uv run pytest tests/bdd/features/analysis/ -v
